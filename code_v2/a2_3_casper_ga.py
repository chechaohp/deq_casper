# -*- coding: utf-8 -*-
"""A2-3-Casper_GA.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1drATNUmWnyc7-NjCy-_HhsJ_JzUI02ry
"""
...
def train_val_test_split(data:DataFrame, test_patient:str, val_patient) -> Tuple[DataFrame,DataFrame,DataFrame]:
    """[summary]

    Args:
        data (DataFrame): [description]
        test_patient (str): [description]
        val_ratio (float, optional): [description]. Defaults to 0.1.

    Returns:
        Tuple[DataFrame,DataFrame,DataFrame]: [description]
    """
    # check input
...
# bitstring[bitstring < theshold] = 0
# bitstring[bitstring >= theshold] = 1
# print(bitstring.sum())
# casper
deq_layer = False
max_layer = 40
    ...
    for patient in tqdm(patient_list):
        ...
        for val_p in patient_list:
            ...
            if val_acc > best_accuracy:
                ...
                torch.save(model.state_dict(), './best_model/{model_name}_{patient}_nodeq.pth'.format(model_name=type(model).__name__, patient=patient))
                best_p_val = val_p

        # load best model to calculate accuracy
        model.load_state_dict(torch.load('./best_model/{model_name}_{patient}_nodeq.pth'.format(model_name=type(model).__name__, patient=patient)))
        model.eval()
        # get train, val, test data
